...
const AuthLock = ({ socket, onAuthenticated, onAnimationComplete }) => {
    ...
    const themeColor = isUnlocking ? 'text-green-500' : 'text-white';
    const borderColor = isUnlocking ? 'border-green-500' : 'border-white';
    const shadowColor = isUnlocking ? 'shadow-[0_0_50px_rgba(34,197,94,0.4)]' : 'shadow-[0_0_50px_rgba(255,255,255,0.2)]';
            ...
            <div className={`relative flex flex-col items-center gap-6 p-10 border ${borderColor}/30 rounded-lg bg-black/80 backdrop-blur-xl ${shadowColor} transition-all duration-[1500ms]`}>
                ...
                <div className={`relative w-64 h-64 border-2 ${borderColor}/50 rounded-lg overflow-hidden bg-gray-900 shadow-inner flex items-center justify-center transition-colors duration-500`}>
                    {frameSrc ? (
                        ...
                        <div className={`animate-pulse ${isUnlocking ? 'text-green-800' : 'text-white/50'}`}>
                            <User size={64} />
                        </div>
                    )}
                    ...
                    {!isUnlocking && (
                        <div className="absolute top-0 left-0 w-full h-1 bg-white/80 shadow-[0_0_15px_white] animate-[scan_2s_ease-in-out_infinite]"></div>
                    )}
